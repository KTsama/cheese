...
class Utils(private val env: Env) {
    fun run() {
        if (!env.runTime.runMode) {
            ...
            HttpUtils.get().getDownload("${env.runTime.ip}/download", "${WORKING_DIRECTORY}/debug.zip")
            ...
            node.run(
                "${JS_DIRECTORY}/main.js",
                js = FilesUtils.get().read("${JS_DIRECTORY}/main.js").toString()
            )
            env.runTime.runState = false
        }
    }
    ...
    XmlUtils.get().parseXml("$UI_DIRECTORY/activity_main.xml")
        XmlUtils.get().startActivity("P1", object : IActivity {
